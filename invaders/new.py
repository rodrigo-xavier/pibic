...
STORE_PATH = "../../.database/pibic/invaders/"
MAX_EPSILON = 1
MIN_EPSILON = 0.1
...
def record_gif(frame_list, episode, fps=50):
    imageio.mimsave(STORE_PATH + "/SPACE_INVADERS_EPISODE-" + str(episode) + ".gif", frame_list, fps=fps) #duration=duration_per_frame)
...
eps = MAX_EPSILON
render = True
train_writer = tf.summary.create_file_writer(STORE_PATH + "/DuelingQSI_ " + str(dt.datetime.now().strftime('%d%m%Y%H%M')))
double_q = True
steps = 0
for i in range(num_episodes):
    ...
    while True:
        ...
        if done:
            if steps > DELAY_TRAINING:
                avg_loss /= cnt
                print("Episode: " + str(i) + ", Reward: " + str(tot_reward) + ", avg loss: " + str(avg_loss) + ", eps: " + str(eps))
                with train_writer.as_default():
                    tf.summary.scalar('reward', tot_reward, step=i)
                    tf.summary.scalar('avg loss', avg_loss, step=i)
            else:
                print("Pre-training...Episode: " + str(i))
            if i % GIF_RECORDING_FREQ == 0:
